...
public class Analyzer {
    ...
    public static List<List<List<Double>>> mutateIntervalSelect(List<PImage> images) throws InterruptedException {
        List[] attrib = calAttribute(images);
        List<Double> imgAvg = attrib[0];
        List<Double> imgSD = attrib[1];

        List<List<Double>> avgOverRange = new LinkedList<>();
        List<List<Double>> sdOverRange = new LinkedList<>();
        ...
        for (int i = 0; i < attrib[1].size(); i++) {
            if (imgAvg.get(i) < imgAvg_lowerbound || imgAvg.get(i) > imgAvg_upperbound) {
                if (img_Avg_adjacent) {
                    avgOverRange.get(avg_clusterIdx).add(imgAvg.get(i));
                } else {
                    img_Avg_adjacent = true;
                    avgOverRange.add(new LinkedList<Double>());
                    avgOverRange.get(++avg_clusterIdx).add(imgAvg.get(i));
                }
            } else {
            ...
            if (imgSD.get(i) < img_sd_lowerbound || imgSD.get(i) > img_sd_upperbound) {
                if (img_SD_adjacent) {
                    sdOverRange.get(sd_clusterIdx).add(imgSD.get(i));
                } else {
                    img_SD_adjacent = true;
                    sdOverRange.add(new LinkedList<Double>());
                    sdOverRange.get(++sd_clusterIdx).add(imgSD.get(i));
                }
            } else {
        ...
        List<List<List<Double>>> result = new LinkedList<>();
